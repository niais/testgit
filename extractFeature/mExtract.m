...
%     imglist = textread([data_root video_name '/annotations.txt']);
    frame_num = total_video_bbs(dn).total_frame_num;
%     video_bbs = total_video_bbs(dn).video_bbs;
    ...
    video_frame_im = {};
    for jf = 1:frame_num
        img_name = [data_root video_name '/frame' num2str(jf,'%.4d') '.jpg'];
        im = imread(img_name);
        video_frame_im{end+1} = im;
    end
    
    person_num = length(tracklet);
    for i = 1:person_num
        ts = clock;
        
        tmp_bbs_trans = [];
        frame_ix = tracklet(i).ti:tracklet(i).te; 
        tmp_bbs = tracklet(i).bbs';        
        bbs_num = size(tmp_bbs,1);
    ...
    %                [tmp_simi(si,sj),~] = get_similarity(old_patch,new_patch);  
        ...
        feats = ExtractFeatsR(new_patchs,net,batch_size);
        %% check
        if bbs_num == size(feats,2)
        ...
        te = clock;
        vte = clock;
        fprintf('%d/%d video %d/%d person %d boxes cost %.4fs total %.2fs\n',...
            dn,num_video,i,person_num,bbs_num,etime(te,ts), etime(vte,vts));
    end

    save([save_root 'Provfeats_' video_name '.mat'],'video_feats');
